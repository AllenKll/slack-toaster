...
class Toast(QWidget):
    ...
    def __init__(self, title, body, link):
        ...
        self.setWindowFlags(self.windowFlags() | QtCore.Qt.Tool | QtCore.Qt.MSWindowsFixedSizeDialogHint)
...
class ScreenManager():
    ...
    def showBox( self, title, body, link, ts):
        ...
        # check for existing TimeStamp and reject duplicates
        if len([x for x in self.toastList if x.ts == ts]) > 0:
            return
